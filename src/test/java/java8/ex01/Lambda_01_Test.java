...
public class Lambda_01_Test {
	...
	private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
		...
		List<Person> personFiltre = new ArrayList<Person>();
...
//		PersonPredicate testAge = new PersonPredicate() {
//			
//			@Override
...
//		};
	...
	public void test_filter_by_password() throws Exception {
		...
		PersonPredicate testPassword = p->DigestUtils.sha512Hex(p.getPassword()).equals(passwordSha512Hex) && p.getAge()>= 49;
										
		List<Person> result = filter(personList, testPassword);
